<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="项目名称" prop="name" sortable="custom" align="center" min-width="120px" class-name="small-padding fixed-width" />
    ...
    <el-dialog
      ...
      width="30%"
    >
      <span>确定要删除项目<b>{{ deleteRowData.name }}</b>吗？</span>
    ...
    <el-dialog :title="dialogStatus" :visible.sync="dialogFormVisible" width="40%">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="90px" class="transfer-class">
        <el-form-item label="项目名称" prop="name">
          <el-input v-model.trim="temp.name" style="width: 70%" placeholder="项目名称" />
        </el-form-item>
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createProject(this.temp, 'post').then(response => {
            this.restTemp()
            this.restData()
            this.getList()
            this.$message({
              title: 'Success',
              message: `项目 ${this.temp.name} 添加成功`,
              type: 'success',
              duration: 2000
            })
          })
    ...
    confirmDjalog() {
      ...
      deleteProject(ID).then(() => {
        ...
        this.$message({
          title: 'Success',
          message: `项目 ${this.deleteRowData.name} 删除成功`,
          type: 'success',
          duration: 2000
        })
      })
