<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.search" placeholder="角色名称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="角色名称" prop="name" sortable="custom" align="center" min-width="120px" class-name="small-padding fixed-width" />
    ...
    <el-dialog
      ...
      width="30%"
    >
      <span>确定要删除角色<b>{{ deleteRowData.name }}</b>吗？</span>
    ...
    <el-dialog :title="dialogStatus" :visible.sync="dialogFormVisible" width="40%">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="90px" class="transfer-class">
        <el-form-item label="角色名称" prop="name">
          <el-input v-model.trim="temp.name" style="width: 70%" placeholder="角色名称" />
        </el-form-item>
...
<script>
import { getProjectRole, createProjectRole, updateProject, deleteProject } from '@/api/projectRole'
...
export default {
  ...
  methods: {
    getList(obj) {
      ...
      getProjectRole(this.listQuery).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createProjectRole(this.temp, 'post').then(response => {
            this.restTemp()
            this.restData()
            this.getList()
            this.$message({
              title: 'Success',
              message: `角色 ${this.temp.name} 添加成功`,
              type: 'success',
              duration: 2000
            })
          })
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateProject(ID, tempData, 'patch').then(() => {
            this.restData()
            this.getList()
            this.$message({
              title: 'Success',
              message: `角色 ${tempData.name} 更新成功`,
              type: 'success',
              duration: 2000
            })
          })
    ...
    confirmDjalog() {
      ...
      deleteProject(ID).then(() => {
        ...
        this.$message({
          title: 'Success',
          message: `角色 ${this.deleteRowData.name} 删除成功`,
          type: 'success',
          duration: 2000
        })
      })
